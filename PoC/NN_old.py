...
oversample_model.fit(Xsm_train, ysm_train, validation_split=0.2,batch_size=100, epochs=50, shuffle=True, verbose=2)


#%%

oversample_predictions = oversample_model.predict(original_Xtest, verbose=0)


#%%

oversample_bankruptcy_predictions = oversample_model.predict_classes(original_Xtest,batch_size=100, verbose=0)
...
n_inputs_FS = Xsm_train_FS.shape[1]

oversample_model_FS = Sequential([
    Dense(n_inputs, input_shape=(n_inputs_FS, ), activation='relu'),
    Dense(80, activation='relu'),
    Dense(1, activation='sigmoid')
])
...
oversample_model_FS.compile(optimizer='sgd', loss='binary_crossentropy', metrics=[Accuracy(), Recall()])

#%%

oversample_model_FS.fit(Xsm_train_FS, ysm_train_FS, validation_split=0.2,batch_size=15, epochs=50, shuffle=True, verbose=2)


#%%

oversample_predictions_FS = oversample_model_FS.predict(original_Xtest_FS, verbose=0)


#%%

oversample_bankruptcy_predictions_FS = oversample_model_FS.predict_classes(original_Xtest_FS,batch_size=15, verbose=0)
